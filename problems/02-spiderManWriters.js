...
const spiderManWriters = (str) => {
    ...
    while (i < words.length) {
        let word = words[i];
        let colonized = words[i-1];
        let preWord = `${words[i-2]}`;
        if (/no/gi.test(word)) {
            let deColonized = colonized.split('')[0];
            let priorWords = `${preWord} ${deColonized}`;
            let deComma = words[i+1].split('');
            deComma.pop();
            let afterWords = `${word} ${deComma.join('')}`;
            Object.assign(obj, {[priorWords]: afterWords});
        }
        i++
    }
    return obj;
...
let str1 = 'Angela T: wrote about Spiderman, Miah B: no Spiderman, Adrian R: wrote about Spiderman';
console.log(spiderManWriters(str1));

let str2 = 'Tom B: no Spiderman, Justin N: no Spiderman, Emily B: wrote about Spiderman';
